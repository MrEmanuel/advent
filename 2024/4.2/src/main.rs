...
fn main()  {
    ...
    for (line_index, line) in read_to_string(file_path).unwrap().lines().enumerate() {
        if line_len == 0 {
            ...
            for column_index in 0..line.len(){
                ...
                if character == 'A' && line_index != 0 && line_index != line_len-1 && column_index != 0 && column_index != line_len-1 {
                    a_pos.push((column_index,line_index))
    ...
    // For each position in A, create 2 words and match them against MAS and SAM

    let mut count= 0;
    for (x,y) in a_pos {
        println!("{x},{y}");
        let val1 = columns.index(x-1).index(y-1);
        let val2 = columns.index(x+1).index(y-1);
        let val3 = columns.index(x-1).index(y+1);
        let val4 = columns.index(x+1).index(y+1);

        let word1 = val1.clone() + "A" + val4;
        let word2 = val2.clone() + "A" + val3;
        ...
        match (&word1[..], &word2[..]) {
            ...
            _ => {
                // println!("No match for {},{}",word1, word2);
            }
        }
